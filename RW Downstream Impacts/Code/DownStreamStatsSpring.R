...
LaGrange = read.csv("LaGrangeSpring.csv")                                     #Illinois River @ La Grange
Flow_LaGrange = LaGrange[["FlowSpring"]]
Stage_LaGrange = LaGrange[["StageSpring"]]
DeFacto_LaGrange = Flow_LaGrange*0.646316889697

Peoria = read.csv("PeoriaSpring.csv")                                         #Illinois River @ Peoria
Flow_Peoria = Peoria[["FlowSpring"]]
Stage_Peoria = Peoria[["StageSpring"]]
DeFacto_Peoria = Flow_Peoria*0.646316889697

StarvedRock = read.csv("StarvedRockSpring.csv")                               #Illinois River @ Starved Rock
Flow_StarvedRock = StarvedRock[["FlowSpring"]]
Stage_StarvedRock = StarvedRock[["StageSpring"]]
DeFacto_StarvedRock = Flow_StarvedRock*0.646316889697

Marseilles = read.csv("MarseillesSpring.csv")                                 #Illinois River @ Marseilles
Flow_marseilles = Marseilles[["FlowSpring"]]
Stage_marseilles = Marseilles[["StageSpring"]]
DeFacto_marseilles = Flow_marseilles*0.646316889697

Dresden = read.csv("DresdenSpring.csv")                                       #Illinois River @ Dresden
Flow_Dresden = Dresden[["FlowSpring"]]
Stage_Dresden = Dresden[["StageSpring"]]
...
PF_Output = file("PF_Output_Spring.csv","w+")                                                                 #Create a output file for probability of failure
...
tTest_Output = file("tTest_Output_Spring.csv","w+")                                                               #Create a output file for t Tests
